...
(defparameter +up-message+ "SERVER_UP")
...
(defparameter +log-levels+ '(:off :fatal :error :warn :info :debug))
...
(defmacro ensure-termination-is-send (stream &body body)
  `(unwind-protect 
      (progn ,@body)
      (progn (write-io-termination ,stream) (log:info "Termination signal send"))))
...
(defmacro in-user-environment (packagevar success-form error-form)
...
(defun shutdown-eval-server ()
  ...
  (setf *is-running* NIL)
  (log:debug "Eval-Server shutdown at ~a" (now-string)))
...
(defun setup-logging (level)
  (not-member->error level +log-levels+)
  (log:config level)
  (log:config :daily "dandelion_log.txt"))
...
(defun connect-client (reader)
  (with-line-read reader line
    (let (connect-args)
      ...
      (socket-stream (socket-connect (second connect-args) (third connect-args))))))
...
(defun report-package-not-found (writer package)
  (log:error "package ~a not found" package)
                    ...
                    (in-read-write-environment reader writer socket-io
                      (handler-bind ((error #'restart-redirect))
                        (in-user-environment package
                                             ...
                                             (progn (report-package-not-found writer package) (return-from handle-eval-request))))))
      (abort-condition (condition) (declare (ignore condition)) (write-ok writer) (log:info "User aborted restart") (return-from handle-eval-request))
      (error (condition) (log:error "Error condition-handle request ignored: ~a" condition)
             (return-from handle-eval-request)))
    
    ;;invariante: Form wurde evaluiert, evaluieren direkt erfolgreich oder durch aufruf eines oder mehrerer Restarts

    (log:info "Eval succesfully completed, result is: ~a" result-list))
   
   ;invariante: Terminierungssignal wurde gesendet
    (write-eval-success writer post-eval-package-string result-list)
    (log:debug "eval-handle done, result written")))
...
(defun handle-loop (reader writer socket-io)
  (log:debug "handle loop entered")
  (loop
   (with-line-read reader line
     (let ((id (read-identifier line)))
       (log:info "handling request: ~a" line)
       (handler-case 
           (cond ((equalp id +token-eval+) 
                 ...
                 ((equalp id +token-disconnect+) 
                  (write-ok writer) (log:info "Client disconnects, stopping handle loop") (return T))) ;bei disconnect laeuft die handle-loop aus
         (malformed-protocol-error (se) (write-error writer "unknown-request ~a" (protocol-string se)) 
                                        (log:info "Illegal/Unknown command: ~a, ignoring request" line)))))))
...
(defun start-handle-loop (socket)
  (let ((reader (make-instance 'protocol-reader :input-stream (socket-stream socket)))
        (writer (make-instance 'protocol-writer :output-stream (socket-stream socket)))
        io-socket)
    (unwind-protect ;stellt sicher das die Stream in jedem Fall geschlossen werden
        (progn ;protected-form
          (log:debug "New Client connected at ~a : ~a" (now-string) (get-peer-address socket))

          ;Client Verbindungsversuch
          (handler-case (setf io-socket (connect-client reader)) ;Connect-Kommando vom Client lesen
            (malformed-protocol-error (se) (log:error "illegal connect: ~a" se) (return-from start-handle-loop)) ;Bei Fehler Schleife beenden
            (error (se) (log:error "unknown error: ~a" se) (return-from start-handle-loop)))
          
          ;Event-Loop
          (handler-case (handle-loop reader writer io-socket)
            (end-of-file (se) (log:error "handle loop unexpected exited: ~a" se))
            (error (se) (log:error "unknown error in handle loop: ~a" se)))
          
          (log:debug "Client disconnected at ~a" (now-string)))
        (progn ;clean-up form
...
; value:  -
(defun start-eval-server (port &key (serve-forever nil) (log-level :error))
  (setf *is-running* T)
  
  (setup-logging log-level)
  
  (log:debug "Eval-Server started at ~a" (now-string))

  (setf *server-socket* (socket-listen "127.0.0.1" port))
  (unwind-protect
      (progn
        (send-startup)
        (loop do
