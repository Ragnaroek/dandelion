...
implements IBackgroundEvaluationListener, ITextListener, IListenerView
{
	...
	throws PartInitException 
	{
		...
		LispCore.getEnvironmentManager().addEvalServerManagementListener(this);
	}
	...
	protected void openListener()
	{
		...
		for(IEnvironment server : LispCore.getEnvironmentManager().getEnvironments()) {
		...
		if(selectedServer != null) {
			try {
				LispCore.getEnvironmentManager().openListenerFor(selectedServer);
	...
	private void fillServerCombo()
	{
		if(fServerCombo != null) { //null wenn secondary != null
			...
			for(IEnvironment server : LispCore.getEnvironmentManager().getEnvironments(true)) {
	...
	private void internalListenerEval()
	{
		...
		try {
			connection = LispCore.getEnvironmentManager().getConnectionFor(fEvalServer);
	...
	public void dispose() {
		...
		LispCore.getEnvironmentManager().removeEvalServerManagementListener(this);
		super.dispose();
	}
