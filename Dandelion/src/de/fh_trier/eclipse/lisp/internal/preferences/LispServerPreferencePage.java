...
import de.fh_trier.eclipse.lisp.internal.ui.dialogs.EnvironmentEditDialog;
...
implements IWorkbenchPreferencePage 
{
	...
	private final EnvironmentManager fManager = LispCore.getEvalServerManager();
	...
	protected Control createContents(final Composite ancestor) 
	{
		...
		fLabelProvider = new LispEvalServerLabelProvider(fManager.getDefaultEvalServer());
	...
	{
		if(fNewDefault == null) {
			return fManager.getDefaultEvalServer();
		}
		return fNewDefault;
	}
	...
	protected List<IEnvironment> getCurrentServerList()
	{
		...
		List<IEnvironment> serverInManager = fManager.getEvalServer();
	...
	protected void removeServer()
	{
		...
		if(selected != null) {
			if(fManager.isUsedInProject(selected)) {
	...
	{	
		for(Entry<IEnvironment, IEnvironmentConfiguration> entry : fAdded.entrySet()) {
			fManager.addEvalServer(entry.getKey(), entry.getValue());
		}
		
		for(IEnvironment removed : fRemoved) {
			try {
				fManager.removeEvalServer(removed);
		...
		if(fNewDefault != null) {
			fManager.setDefaultServer(fNewDefault);
		...
		for(IEnvironment server : fManager.getEvalServer()) {
