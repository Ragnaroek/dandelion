...
import de.fh_trier.eclipse.lisp.internal.preferences.LispProjectPreferences;
...
implements INewWizard
{
	...
	private void createLispProject() 
	{
		...
		try {
			...
			LispProjectPreferences.storeEvalServer(project, selected);
		    LispCore.getEvalServerManager().setEvalServerFor(project, selected);
		} catch (CoreException e) {
			LispUI.showErrorDialog(getShell(), "Error creating project", e);
		} catch (ConnectionException e) {
			LispUI.showErrorDialog(getShell(), "Error setting connection", e);
		}
	}
