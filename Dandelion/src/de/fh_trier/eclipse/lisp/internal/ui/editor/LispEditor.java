...
import de.fh_trier.eclipse.lisp.internal.core.connection.ManagementException;
...
{	
	...
	private IAction fOpenListenerAction = new Action("Open Listener") {
		...
		public void run() {
			try {
				LispCore.getEvalServerManager().openListenerFor(getInput().getProject());
			} catch (PartInitException e) {
				LispUI.showErrorDialog(getSite().getShell(), e);
			} catch (ManagementException e) {
				LispPluginActivator.logBrokenInvariant("no eval server set for this project", e);
			}
		}
	};
