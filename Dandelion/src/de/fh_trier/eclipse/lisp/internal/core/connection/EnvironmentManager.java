...
import de.fh_trier.eclipse.lisp.internal.preferences.*;
import de.fh_trier.eclipse.lisp.internal.ui.views.listener.*;
import edu.umd.cs.findbugs.annotations.*;

/**
 * Die zentrale Verwaltungsklasse fuer die Lisp-Umgebungen.
 * Ein Zugriff auf die Singleton-Instanz ist ueber {@link LispCore#getEvalServerManager()}
...
implements IDisposable, IResourceChangeListener, IJobHandler
{	
	...
	/**
	 * Die Singleton-Instanz ist ueber {@link LispCore#getEvalServerManager()} zu erreichen.
	...
	{	
		...
		for(IProject project : projects) {
			if( hasLispNature(project) ) {
				try {
					IEnvironment evalServer = LispProjectPreferences.readEvalServer(project);
					fProjectAssociation.put(project, evalServer);
				} catch (CoreException e) {
					LispPluginActivator.logError("error reading preferences for project: " + project.getName(), e);
				}
			}
		}
	 ...
	 * @param config
	 */
	public synchronized void addEvalServer(final IEnvironment server, final IEnvironmentConfiguration config)
	{
		...
		if(fEvalServer.size() == 0) { //der erste Server der hinzugefuegt wurde
			setDefaultServer(server); //als default setzen
		}
	 ...
	 */
	public synchronized void removeEvalServer(final IEnvironment server)
	throws ManagementException, ConnectionException
	{
		if(isUsedInProject(server)) {
	 ...
	 * @throws ManagementException - wenn IEvalServer nicht bekannt (nicht aus Liste {@link EnvironmentManager#getEvalServer()}
	 */
	@NonNull
	...
	throws ManagementException
	{
		...
		IEnvironment server = getEvalServerFor(project);
		if(server == null) {
	...
	{
		return getSymbolStoreFor(getEvalServerFor(project));
	}
	...
	private boolean hasLispNature(final IProject project) throws CoreException
	{
	 ...
	 */
	public synchronized IEnvironment setEvalServerFor(final IProject project, final IEnvironment server) 
	throws ConnectionException
	{	
	 ...
	 * Lisp-Projekt ist.
	 */
	public synchronized IEnvironment getEvalServerFor(final IProject project)
	{
	...
	{
		IEnvironment evalServer = getEvalServerFor(project);
		if(evalServer == null) {
			throw new ManagementException("Eval server for this project not found");
		}
	 ...
	 */
	public synchronized List<IEnvironment> getEvalServer()
	{
		return new ArrayList<IEnvironment>(fEvalServer);
	}
	
	/**
	 * Wie {@link EnvironmentManager#getEvalServer()} nur sind die Lisp-Umgebung nach Name sortiert.
	 ...
	 */
	public synchronized List<IEnvironment> getEvalServer(boolean sort)
	{
		List<IEnvironment> server = getEvalServer();
		if(sort) {
	 ...
	 * Garantiert nicht <code>null</code> wenn {@link EnvironmentManager#hasEvalServer()} <code>true</code> liefert.
	 * @return Die Default Lisp-Umgebung.
	 * @see EnvironmentManager#hasEvalServer()
	 */
	public synchronized IEnvironment getDefaultEvalServer()
	{
	 ...
	 * @param server
	 */
	public synchronized void setDefaultServer(final IEnvironment server)
	{
	 ...
	 * von {@link EnvironmentManager#getDefaultEvalServer()} <code>null</code> und
	 * {@link EnvironmentManager#getEvalServer()} eine leere Liste.
	 * @return
	 * @see EnvironmentManager#getEvalServer()
	 * @see EnvironmentManager#getDefaultEvalServer()
	 */
	public synchronized boolean hasEvalServer()
	{
	...
	public synchronized void resourceChanged(final IResourceChangeEvent event) 
	{
		if( event.getType() == IResourceChangeEvent.PRE_CLOSE ) {
			...
			projectDeleted((IProject)event.getResource());
	...
	private void projectDeleted(final IProject project)
	{
		//kein nature check
		...
		SaveListenerNotifier notifier = new SaveListenerNotifier(new SaveRunnable() {
			...
			public void run0() {
				try {
					returnConnection(project);
				} catch (ConnectionException e) {
					LispPluginActivator.logError("connection error while deleting project", e);
				}
			}
		});
		...
		SaveListenerNotifier notifier = new SaveListenerNotifier(new SaveRunnable() {
			...
			public void run0() {
				try {
					...
					LispPluginActivator.logError("connection error while closing project", e);
				}
			}
		});
		notifier.execInUIDisplay();
	...
	private void projectOpened(final IProject project) 
	{
		try {
			if( hasLispNature(project) ) {
				IEnvironment server = LispProjectPreferences.readEvalServer(project);
				if(fEvalServer.contains(server)) {
					setEvalServerFor(project, server);
				} else {
					if(hasEvalServer()) {
						setEvalServerFor(project, getDefaultEvalServer());
					}
				}
			}
		} catch (CoreException e) {
			...
			LispPluginActivator.logError("error connecting while opening lisp project", e);
		}
	}
	
	//------ Ende Interface IResourceChangeListener
